...
def _build_compact_banner() -> str:
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Giantect Empire[/]\n"
...
class HermesCLI:
    ...
    def _enable_voice_live(self):
        """Run the real-time voice conversation mode."""
        try:
            from tools.conversation_mode import start_conversation_mode
            start_conversation_mode()
        except Exception as e:
            _cprint(f"\n{_DIM}Conversation Mode Error: {e}{_RST}")
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        try:
            ...
            if self._voice_mode and isinstance(message, str):
                _voice_prefix = (
                    "[Voice mode — respond naturally in 1-3 sentences. "
                    "Match the user's language. No markdown, no code blocks, "
                    "no bullet points. Be conversational like a friend.] "
                )
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Tchuekam Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Tchuekam Agent! Type your message or /help for commands."
