...
export function DesktopOnboardingOverlay({ enabled, onCompleted, requestGateway }: DesktopOnboardingOverlayProps) {
  ...
  // NEVER show this onboarding overlay on startup. Only show it in manual mode
  // (when the user explicitly opened the provider selector via Settings -> Providers).
  ...
  return (
    <div className="border-b border-(--ui-stroke-tertiary) bg-(--ui-chat-bubble-background) px-5 py-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 className="text-[0.9375rem] font-semibold tracking-tight">Let's get you setup with Tchuekam Agent</h2>
...
function FlowPanel({ ctx, flow }: { ctx: OnboardingContext; flow: OnboardingFlow }) {
  ...
  if (flow.status === 'awaiting_browser') {
    return (
      <Step title={`Sign in with ${title}`}>
        <p className="text-sm text-muted-foreground">
          We opened {title} in your browser. Authorize Tchuekam there and you'll be connected automatically — nothing to
          copy or paste.
        </p>
