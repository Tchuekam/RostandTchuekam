...
def _pulse_socket_reachable() -> bool:
    ...
    (issue #35622).
    """
    # PulseAudio/PipeWire do not exist on native Windows.
...
def detect_audio_environment() -> dict:
    ...
    if any(os.environ.get(v) for v in ('SSH_CLIENT', 'SSH_TTY', 'SSH_CONNECTION')):
        if has_forwarded_audio:
            ...
            warnings.append(
                ...
                "  Audio over SSH is not supported on Windows."
            )
        else:
            warnings.append(
                "Running over SSH -- no audio devices available.\n"
                "  If a sound server (PulseAudio/PipeWire) is running on this host,\n"
                "  point it at the client, e.g.:\n"
    ...
    # Skip entirely on native Windows (no /proc filesystem there).
    if sys.platform != "win32":
        try:
...
def play_audio_file(file_path: str) -> bool:
    ...
    if system == "Windows":
        # winsound (stdlib) — synchronous, no extra deps, WAV only.
        if file_path.endswith(".wav"):
            try:
                import winsound
                winsound.PlaySound(os.path.abspath(file_path), winsound.SND_FILENAME)
                return True
            except Exception as e:
                logger.debug("winsound playback failed: %s", e)
        # PowerShell WPF MediaPlayer — handles MP3, WAV, and other formats.
        abs_path = os.path.abspath(file_path).replace("\\", "/")
        ps_cmd = (
            "Add-Type -AssemblyName presentationCore;"
            "$p=New-Object System.Windows.Media.MediaPlayer;"
            f"$p.Open([Uri]::new('file:///{abs_path}'));"
            "$sw=[System.Diagnostics.Stopwatch]::StartNew();"
            "while(-not $p.NaturalDuration.HasTimeSpan -and $sw.Elapsed.TotalSeconds -lt 5)"
            "{[System.Threading.Thread]::Sleep(100)};"
            "if($p.NaturalDuration.HasTimeSpan){"
            "$p.Play();"
            "[System.Threading.Thread]::Sleep([int]($p.NaturalDuration.TimeSpan.TotalMilliseconds)+600)"
            "};"
            "$p.Stop();$p.Close()"
        )
        players.append([
