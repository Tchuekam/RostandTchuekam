...
def _dispatch_synthesis(
    ...
    """Helper to dispatch speech synthesis to a specific provider."""
    ...
    if provider == "elevenlabs":
        ...
        if edge_available:
            ...
            raise ValueError("No TTS provider available (Edge-TTS and NeuTTS unavailable)")
...
def text_to_speech_tool(
    ...
    # Detect if text is in a local language or contains local phrases,
    # and convert it to phonemes for natural speech synthesis
    try:
        from tools.local_languages_helper import detect_language, convert_to_phonemes
        detected_lang = detect_language(text)
        if detected_lang != "auto":
            phonetic_text = convert_to_phonemes(text, detected_lang)
            logger.info(
                "[TTS] Converted local language '%s' to phonetic representation: '%s'",
                detected_lang, phonetic_text
            )
            text = phonetic_text
    except Exception as exc:
        logger.warning("Cameroonian TTS phonetic hook failed: %s", exc)
    ...
    try:
        # Generate audio with the configured provider and local fallback chain
        try:
            file_str = _dispatch_synthesis(
                provider, text, file_str, tts_config, command_provider_config
            )
            # Check the file was actually created
            if not os.path.exists(file_str) or os.path.getsize(file_str) == 0:
                raise RuntimeError("TTS generation produced empty or missing file")
        except Exception as exc:
            # If the primary provider is network-based or failed, try local fallback
            is_cloud = provider in {"edge", "elevenlabs", "openai", "minimax", "xai", "mistral", "gemini"} or (command_provider_config is None and provider not in BUILTIN_TTS_PROVIDERS)
            if is_cloud:
                logger.warning(
                    "Primary TTS provider '%s' failed (%s) — attempting local offline fallback...",
                    provider, exc
                )
                fallback_success = False
                for local_prov in ["piper", "kittentts", "neutts"]:
                    try:
                        logger.info("Trying local fallback provider '%s'...", local_prov)
                        fallback_path = _dispatch_synthesis(
                            local_prov, text, file_str, tts_config, None
                        )
                        if os.path.exists(fallback_path) and os.path.getsize(fallback_path) > 0:
                            logger.info("Local fallback to '%s' succeeded.", local_prov)
                            file_str = fallback_path
                            provider = local_prov  # Update provider context
                            fallback_success = True
                            break
                    except Exception as fallback_exc:
                        logger.warning(
                            "Local fallback to '%s' failed: %s", local_prov, fallback_exc
                        )
