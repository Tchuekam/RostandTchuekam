...
def _process_transcription_result(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Detect Cameroonian local languages
    try:
        from tools.local_languages_helper import detect_language, translate_local_to_standard
        detected_lang = detect_language(transcript)
        if detected_lang != "auto":
            translated = translate_local_to_standard(transcript, detected_lang)
            logger.info(
                "[Transcription] Translated local language '%s' to standard: '%s'",
                detected_lang, translated
            )
            result["transcript"] = translated
            result["detected_local_language"] = detected_lang
    except Exception as exc:
        logger.warning("Cameroonian LID/Translation hook failed: %s", exc)
    return result


def transcribe_audio(file_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    result = _transcribe_audio_impl(file_path, model)
    return _process_transcription_result(result)
