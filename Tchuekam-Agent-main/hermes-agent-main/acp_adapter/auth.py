...
def build_auth_methods() -> list[Any]:
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Tchuekam Agent using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Tchuekam Agent provider",
            description=(
                "Open Tchuekam Agent's interactive model/provider setup in a terminal. "
                "Use this when Tchuekam Agent has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
