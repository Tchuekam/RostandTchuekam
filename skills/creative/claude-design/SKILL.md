...
### Special Case: The User Has a Reference Image/Video Folder

When the user says "I have a folder of design references/inspo — figure out what style I want," **do not ask them to describe it**. Instead, use the `references/visual-audit-design-direction.md` reference file in this skill. The workflow is:

1. Get the folder path from the user
2. Catalog all image/video files by extension and meaningful filename
3. Classify across visual axes (dark/light, accent colors, density, effects, content types)
4. Count clusters and weight by intentionality
5. Synthesize a structured design brief covering color palette, typography, layout, and motion

This is a distinct skill from `user-profiling` (which profiles the user from filesystem artifacts) — this one profiles *their design taste* from a curated reference collection.
