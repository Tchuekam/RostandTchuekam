...
**If the user has a folder of reference images/videos** — before asking questions, analyze the folder using the visual audit method (see `claude-design` skill's `references/visual-audit-design-direction.md`). Extract the design DNA (dark/light, accent colors, typography direction, layout preferences) from their saved references, then confirm your findings rather than starting from zero.

**If no reference folder exists**, get three things — one question at a time, not all at once:
