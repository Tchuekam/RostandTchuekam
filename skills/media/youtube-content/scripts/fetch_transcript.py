...
def fetch_transcript(video_id: str, languages: list = None):
    ...
    Handles both v1.x and v0.x of youtube-transcript-api.
    """
    try:
    ...
    # Try v1.x API first (fetch method)
    try:
        if languages:
        ...
        # v0.x API (get_transcript method)
        pass
    except Exception:
        raise

    # Try v0.x API (get_transcript method)
    try:
        if languages:
            result = api.get_transcript(video_id, languages=languages)
        else:
...
def main():
    ...
    try:
        ...
        elif "no transcript" in error_msg.lower() or "not available" in error_msg.lower():
            print(json.dumps({"error": "No transcript found. Try specifying a language with --language, or the video may have no captions at all."}))
        elif "subtitles" in error_msg.lower():
            print(json.dumps({"error": "No captions/subtitles available for this video."}))
        else:
            print(json.dumps({"error": error_msg}))
        sys.exit(1)

    if not segments:
        print(json.dumps({"error": "No transcript segments returned. Video likely has no captions."}))
        sys.exit(1)
